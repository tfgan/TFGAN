...
def sliding_window_embedding_tensor(time_series, dimension, time_delay):
    """
    Create a sliding-window embedding of a 1D time series using PyTorch.

    Args:
        time_series (torch.Tensor): 1D tensor of length L.
        dimension (int): Window size (embedding dimension).
        time_delay (int): Step size between windows.

    Returns:
        torch.Tensor: Embedded data of shape (n_windows, dimension).
    """
    # Ensure input is 1D
    if time_series.dim() != 1:
        raise ValueError("time_series must be a 1D tensor")

    L = time_series.size(0)
    # Number of windows that fit
    n_windows = (L - dimension) // time_delay + 1

    # Use unfold to build sliding windows
    # unfold(dim, size, step) returns shape (n_windows, dimension)
    embedded = time_series.unfold(0, dimension, time_delay)

    return embedded

def ripser_tensor(emb, maxdim=0):
    """
    Compute 0-dimensional persistent homology (H0) via differentiable MST (Prim's algorithm) on embeddings.

    Args:
        emb (torch.Tensor): Tensor of shape (N, D), requires_grad if needed.
        maxdim (int): Only H0 implemented.

    Returns:
        dict: {'dgms': [dgm0]} with dgm0 as (N,2) tensor on same device.
    """
    N = emb.size(0)
    device = emb.device

    # Pairwise distances, keeps gradient flowing
    dists = torch.cdist(emb, emb, p=2)

    # Prim's algorithm initialization
    visited = torch.zeros(N, dtype=torch.bool, device=device)
    visited[0] = True
    # Distance to MST for each node
    min_dist = dists[0].clone()

    # Store MST edge weights
    ...
    for i in range(N - 1):
        ...
        # Set distances of visited nodes to large value
        # d_masked = min_dist.clone()
        # d_masked[visited] = float('inf')
        d_masked = min_dist.masked_fill(visited, float('inf'))
        # d_masked = d_masked.masked_fill(visited, float('inf'))
        w, idx = torch.min(d_masked, dim=0)
        weights[i] = w
        # Mark new node visited
        # visited[idx] = True
        visited = visited.scatter(0, idx.unsqueeze(0), torch.tensor([True], device=device))
        # Update min_dist: for each unvisited, pick min(current, dist from idx)
        new_dists = dists[idx]
        min_dist = torch.where(new_dists < min_dist, new_dists, min_dist)

    # Sort weights for death times
    death_times, _ = torch.sort(weights)

    # Build diagram
    births = torch.zeros_like(death_times)
    dgm0 = torch.stack([births, death_times], dim=1)
    infinite = torch.tensor([[0.0, float('inf')]], device=device)
    dgm0 = torch.cat([dgm0, infinite], dim=0)
    return {'dgms': [dgm0]}

def wasserstein_tensor(dgm1, dgm2):

    distance = dgm1[:,1] - dgm2[:,1]
    distance = torch.abs(distance)
    distance = torch.nan_to_num(distance, nan=0)
    return torch.sum(distance)

def compute_tda_wasserstein_loss_tensor(fake_series, real_series, embedding_dim=3, time_delay=7):
    # Convert to NumPy
    fake_np = fake_series.flatten()
    real_np = real_series.flatten()
    if fake_np.shape[0] % embedding_dim != 0:
        return 0
    # Embedding
    emb_fake = sliding_window_embedding_tensor(fake_np, embedding_dim, time_delay)
    emb_real = sliding_window_embedding_tensor(real_np, embedding_dim, time_delay)
    # Compute diagrams
    # dgms_fake = ripser_tensor(emb_fake)['dgms']
    # dgms_real = ripser_tensor(emb_real)['dgms']
    dgms_fake = ripser_tensor(emb_fake)['dgms']
    dgms_real = ripser_tensor(emb_real)['dgms']
    # Remove infs
    dgms_fake[0] = dgms_fake[0][torch.isfinite(dgms_fake[0][:, 1])]
    dgms_real[0] = dgms_real[0][torch.isfinite(dgms_real[0][:, 1])]
    # Wasserstein for H0
    if len(dgms_fake[0]) == 0 or len(dgms_real[0]) == 0:
        return 0.0
    tda_loss = wasserstein_tensor(dgms_fake[0], dgms_real[0])
    return tda_loss
...
def excessreturns(dataloc, stock, etf, plotcheck = False):
    ...
    smp = (dates_dt < d1) & (dates_dt > d2)
    s_df = s_df[smp]
    dates_dt = pd.to_datetime(s_df['date'])
    e_df = e_df[smp]
    # Find indexes where neither dataframe has NaNs
    valid_idx = ~(s_df.isna().any(axis=1) | e_df.isna().any(axis=1))
    # Apply the valid index mask (preserving original indexes)
    s_df = s_df[valid_idx].reset_index(drop=True)
    e_df = e_df[valid_idx].reset_index(drop=True)
...
def Evaluation2(ticker,freq,gen,test_data, val_data, h,l,pred,hid_d,hid_g, z_dim, lrg, lrd, n_epochs, losstype, sr_val, device, plotsloc, f_name, plot = False):
    ...
    with torch.no_grad():
        ...
        generated1 = torch.empty([1,1,1,ntest,10000])
...
#             print(generated1.shape)
    ...
    means = mn1.detach().cpu().numpy()
    # reals = np.array(rl1.detach())
    reals = rl1.detach().cpu().numpy()
...
def TFGradientCheck(ticker, gen, disc, gen_opt, disc_opt, criterion, n_epochs, train_data, batch_size, hid_d, hid_g,
                  z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10, pred=1, diter=1, tanh_coeff=100, device='cpu',
                  plot=False, tda_embedding_dim=3, tda_time_delay=3):
    """
    Gradient norm check
    """
    ntrain = train_data.shape[0]
    ...
    SR_norm = torch.empty(nbatches * n_epochs, device=device)
    STD_norm = torch.empty(nbatches * n_epochs, device=device)
    TDA_norm = torch.empty(nbatches * n_epochs, device=device)
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            SR = (torch.mean(PnL_s)) / (torch.std(PnL_s))
            STD = torch.std(PnL_s)
            TDA = compute_tda_wasserstein_loss_tensor(
                fake, real,
                embedding_dim=tda_embedding_dim,
                time_delay=tda_time_delay)
            if TDA == 0:
                TDA_norm[epoch * nbatches + i] = 1
                SR_norm[epoch * nbatches + i] = 1
                PnL_norm[epoch * nbatches + i] = 1
                MSE_norm[epoch * nbatches + i] = 1
                STD_norm[epoch * nbatches + i] = 1
                BCE_norm[epoch * nbatches + i] = 1
                continue
            gen_opt.zero_grad()
            SR.backward(retain_graph=True)
            total_norm = 0
            ...
            gen_opt.zero_grad()
            TDA.backward(retain_graph=True)
            total_norm = 0
            for p in gen.parameters():
                param_norm = p.grad.detach().data.norm(2)
                total_norm += param_norm.item() ** 2
                total_norm = total_norm ** 0.5
            TDA_norm[epoch * nbatches + i] = total_norm
    ...
    gamma = torch.mean(BCE_norm / SR_norm)
    delta = torch.mean(BCE_norm / STD_norm)
    lambda_tda= torch.mean(BCE_norm / TDA_norm)
    print("Completed. ")
    print(r"$\alpha$:", alpha)
    print(r"$\beta$:", beta)
    print(r"$\gamma$:", gamma)
    print(r"$\delta$:", delta)
    print(r"$\lambda$:", lambda_tda)
    ...
    if plot:
        ...
        plt.figure(ticker + " TDA norm")
        plt.title(ticker + " TDA norm")
        plt.plot(range(len(BCE_norm)), TDA_norm)
        plt.ylabel(r"$L^2$ norm")
        plt.xlabel("iteration")
        plt.show()
    ...
    return gen, disc, gen_opt, disc_opt, alpha, beta, gamma, delta, lambda_tda
...
def TrainLoopForGAN(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot = False):
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            #     print(f"D(loss)  item={dloss}")
            #     print(f"G(loss)  item={gloss}")
            #     # After you compute disc_real_pred and disc_fake_pred (before loss and backward):
            #     real_scores = disc_real_pred.detach()
            #     fake_scores = disc_fake_pred.detach()
            #     print(
            #         f"D(real)  min={real_scores.min():.3f}  max={real_scores.max():.3f}  mean={real_scores.mean():.3f}")
            #     print(
            #         f"D(fake)  min={fake_scores.min():.3f}  max={fake_scores.max():.3f}  mean={fake_scores.mean():.3f}")
            #     print("TDA grad norm:", sum(p.grad.norm() * 2 for p in gen.parameters()) * 0.5)
...
def TrainLoopForGANTDA(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, lambda_tda, tda_embedding_dim,
                    tda_time_delay, n_epochs, checkpoint_epoch,
                    train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10,
                    pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
    Training loop for the BCE GAN (ForGAN)
    """
    ntrain = train_data.shape[0]
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            if TDA == 0:
                continue
            gen_loss = criterion(disc_fake_pred, torch.ones_like(disc_fake_pred)) + lambda_tda * TDA
            gen_loss.backward()
    ...
    if plot:
        plt.figure("Disc pred ForGAN TDA")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss ForGAN TDA")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()
...
def TrainLoopMainPnLnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot = False):
    ...
    # print("PnL val (best):", PnL_best)
    return gen, disc, gen_opt, disc_opt
def TrainLoopMainPnLTDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, lambda_tda,
                       tda_embedding_dim, tda_time_delay, n_epochs, checkpoint_epoch,
                       train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1,
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            if TDA == 0:
                continue
            gen_loss = criterion(disc_fake_pred, torch.ones_like(disc_fake_pred)) - alpha * PnL + lambda_tda * TDA
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
    ...
    if plot:
        plt.figure("Disc pred PnL TDA")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss PnL TDA")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()

        plt.figure("Disc loss PnL TDA")
        plt.title("Disc loss")
        plt.plot(range(len(discloss)), discloss)
        plt.show()
...
def TrainLoopMainPnLMSEnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot=False):
    ...
    # print("PnL val (best):", PnL_best)
    return gen, disc, gen_opt, disc_opt
def TrainLoopMainPnLMSETDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, lambda_tda,
                          tda_embedding_dim, tda_time_delay, n_epochs,
                          checkpoint_epoch, train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001,
                          lr_g=0.0001, h=1, l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
    Training loop: PnL and MSE loss
    """
    ntrain = train_data.shape[0]
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            if TDA == 0:
                continue
            gen_loss = criterion(disc_fake_pred, torch.ones_like(disc_fake_pred)) - alpha * PnL + beta * SqLoss + lambda_tda * TDA
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
    ...
    if plot:
        plt.figure("Disc pred PnL MSA TDA")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss PnL MSA TDA")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()

        plt.figure("Disc loss PnL MSA TDA")
        plt.title("Disc loss")
        plt.plot(range(len(discloss)), discloss)
        plt.show()
...
def TrainLoopMainPnLMSESRnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot = False):
    ...
    # print("PnL val (best):", PnL_best)
    return gen, disc, gen_opt, disc_opt
def TrainLoopMainPnLMSESRTDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, lambda_tda,
                               tda_embedding_dim, tda_time_delay, n_epochs,
                            checkpoint_epoch, train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001,
                            lr_g=0.0001, h=1, l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            gen_loss = criterion(disc_fake_pred,
                                 torch.ones_like(disc_fake_pred)) - alpha * PnL + beta * SqLoss - gamma * SR + lambda_tda * TDA
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
    ...
    if plot:
        plt.figure("Disc pred PnL MSE SR TDA")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss PnL MSE SR TDA")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()

        plt.figure("Disc loss PnL MSE SR TDA")
        plt.title("Disc loss")
        plt.plot(range(len(discloss)), discloss)
        plt.show()
...
def TrainLoopMainPnLMSESTDnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot = False):
    ...
    # print("PnL val (best):", PnL_best)
    return gen, disc, gen_opt, disc_opt
def TrainLoopMainPnLMSESTDTDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, lambda_tda,
                             tda_embedding_dim, tda_time_delay, n_epochs,
                             checkpoint_epoch, train_data, validation_data, batch_size, hid_d, hid_g, z_dim,
                             lr_d=0.0001, lr_g=0.0001, h=1, l=10, pred=1, diter=1, tanh_coeff=100, device='cpu',
                             plot=False):
    """
    Training loop: PnL, MSE, STD loss
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            if TDA == 0:
                continue
            gen_loss = criterion(disc_fake_pred,
                                 torch.ones_like(disc_fake_pred)) - alpha * PnL + beta * SqLoss + delta * torch.std(
                PnL_s) + lambda_tda * TDA
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
    ...
    if plot:
        plt.figure("Disc pred PnL MSE STD TDA")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss PnL MSE STD TDA")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()

        plt.figure("Disc loss PnL MSE STD TDA")
        plt.title("Disc loss")
        plt.plot(range(len(discloss)), discloss)
        plt.show()
...
def TrainLoopMainPnLSRnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot = False):
    ...
    return gen, disc, gen_opt, disc_opt
def TrainLoopMainPnLSRTDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, lambda_tda,
                         tda_embedding_dim, tda_time_delay, n_epochs, checkpoint_epoch,
                         train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1,
                         l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            if TDA == 0:
                continue
            gen_loss = criterion(disc_fake_pred, torch.ones_like(disc_fake_pred)) - alpha * PnL - gamma * SR + lambda_tda * TDA
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
    ...
    if plot:
        plt.figure("Disc pred PnL SR TDA")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss PnL SR TDA")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()

        plt.figure("Disc loss PnL SR TDA")
        plt.title("Disc loss")
        plt.plot(range(len(discloss)), discloss)
        plt.show()
...
def TrainLoopMainMSEnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot = False):
    ...
    # print("PnL val (best):", PnL_best)
    return gen, disc, gen_opt, disc_opt
def TrainLoopMainMSETDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta,  lambda_tda,
                         tda_embedding_dim, tda_time_delay, n_epochs, checkpoint_epoch,
                       train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1,
                       l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            if TDA == 0:
                continue
            gen_loss = criterion(disc_fake_pred, torch.ones_like(disc_fake_pred)) + beta * SqLoss + lambda_tda * TDA
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
    ...
    if plot:
        plt.figure("Disc pred MSE TDA")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss MSE TDA")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()

        plt.figure("Disc loss MSE TDA")
        plt.title("Disc loss")
        plt.plot(range(len(discloss)), discloss)
        plt.show()
...
def TrainLoopMainSRnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot = False):
    ...
    # print("PnL val (best):", PnL_best)
    return gen, disc, gen_opt, disc_opt
def TrainLoopMainSRTDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, lambda_tda,
                      tda_embedding_dim, tda_time_delay, n_epochs, checkpoint_epoch,
                      train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1, l=10,
                      pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            gen_loss = criterion(disc_fake_pred, torch.ones_like(disc_fake_pred)) - gamma * SR + lambda_tda * TDA
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
    ...
    if plot:
        plt.figure("Disc pred SR TDA")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss SR TDA")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()

        plt.figure("Disc loss SR TDA")
        plt.title("Disc loss")
        plt.plot(range(len(discloss)), discloss)
        plt.show()
...
def TrainLoopMainSRMSEnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot = False):
    ...
    # print("PnL val (best):", PnL_best)
    return gen, disc, gen_opt, disc_opt
def TrainLoopMainSRMSETDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta,  lambda_tda,
                         tda_embedding_dim, tda_time_delay, n_epochs, checkpoint_epoch,
                         train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001, lr_g=0.0001, h=1,
                         l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            gen_loss = criterion(disc_fake_pred, torch.ones_like(disc_fake_pred)) + beta * SqLoss - gamma * SR + lambda_tda * TDA
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
    ...
    if plot:
        plt.figure("Disc pred SR MSE")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss SR MSE")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()

        plt.figure("Disc loss SR MSE")
        plt.title("Disc loss")
        plt.plot(range(len(discloss)), discloss)
        plt.show()
...
def TrainLoopMainPnLSTDnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, n_epochs, checkpoint_epoch, train_data, validation_data, batch_size,hid_d, hid_g, z_dim, lr_d = 0.0001, lr_g = 0.0001, h = 1, l = 10, pred = 1, diter =1, tanh_coeff = 100, device = 'cpu', plot = False):
    ...
    return gen, disc, gen_opt, disc_opt
def TrainLoopMainPnLSTDTDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma, delta, lambda_tda,
                          tda_embedding_dim, tda_time_delay, n_epochs,
                          checkpoint_epoch, train_data, validation_data, batch_size, hid_d, hid_g, z_dim, lr_d=0.0001,
                          lr_g=0.0001, h=1, l=10, pred=1, diter=1, tanh_coeff=100, device='cpu', plot=False):
    """
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        for i in range(nbatches):
            ...
            if TDA == 0:
                continue
            gen_loss = criterion(disc_fake_pred, torch.ones_like(disc_fake_pred)) - alpha * PnL + delta * STD + lambda_tda * TDA
            gen_loss.backward()
            gen_opt.step()
            gloss = gen_loss.detach().item()
            genloss[epoch * nbatches + i] = gloss
    ...
    if plot:
        plt.figure("Disc pred PnL STD TDA")
        plt.plot(range(len(dscpred_fake)), dscpred_fake, alpha=0.5, label='generated')
        plt.plot(range(len(dscpred_fake)), dscpred_real, alpha=0.5, label='real')
        plt.legend(loc='best')
        plt.show()

        plt.figure("Gen loss PnL STD TDA")
        plt.title("Gen loss")
        plt.plot(range(len(genloss)), genloss)
        plt.show()

        plt.figure("Disc loss PnL STD TDA")
        plt.title("Disc loss")
        plt.plot(range(len(discloss)), discloss)
        plt.show()
...
def TFGAN_combos(ticker, loc, modelsloc, plotsloc, dataloc, etflistloc, vl_later=True, lrg=0.0001, lrd=0.0001,
                  n_epochs=500, ngrad=100, h=1, l=10, pred=1, ngpu=1, tanh_coeff=100, tr=0.8, vl=0.1, z_dim=32,
                  hid_d=64, hid_g=8, checkpoint_epoch=20, batch_size=100, diter=1, plot=False, freq=2):
    """
    ...
    if ticker[0] == 'X':
        train_data, val_data, test_data, dates_dt = split_train_val_testraw(ticker, dataloc, tr, vl, h, l, pred,
                                                                            plotcheck=False)
    else:
        train_data, val_data, test_data, dates_dt = split_train_val_test(ticker, dataloc, etflistloc, tr, vl, h, l,
                                                                         pred, plotcheck=False)
    ...
    gen = Generator(noise_dim=z_dim, cond_dim=condition_size, hidden_dim=hid_g, output_dim=pred, mean=ref_mean,
                    std=ref_std)
    gen.to(device)
    ...
    gen, disc, gen_opt, disc_opt, alpha, beta, gamma, delta, lambda_tda = TFGradientCheck(ticker, gen, disc, gen_opt, disc_opt,
                                                                            criterion, ngrad, train_data, batch_size,
                                                                            hid_d, hid_g, z_dim, lrd, lrg, h, l, pred,
                                                                            diter, tanh_coeff, device, plot, tda_embedding_dim, tda_time_delay)
    ...
    print("PnL TDA")
    losstype = "PnL TDA"
    genPnLTDA, discPnLTDA, gen_optPnLTDA, disc_optPnLTDA = TrainLoopMainPnLTDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta,
                                                                  gamma, delta, lambda_tda, tda_embedding_dim,
                                                                  tda_time_delay, n_epochs, checkpoint_epoch, train_data,
                                                                  validation_data, batch_size, hid_d, hid_g, z_dim, lrd,
                                                                  lrg, h, l, pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLTDA.state_dict()}, f_name + "PnLTDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genPnLTDA,
                                                                                                          test_data,
                                                                                                          validation_data,
                                                                                                          h, l, pred,
    ...
    pd.DataFrame(PnL_test[PnL_test_cnt]).to_csv(loc + "PnLs/" + ticker + "-FinGAN-" + losstype + ".csv")
    ...
    plt.ylabel("bpts")
    plt.plot(dates_dt[-int(ntest / 2):], np.cumsum(PnL_test[PnL_test_cnt]), label="PnL")
    plt.legend(loc='best')
    PnL_test_cnt += 1
    ...
    # region PnL MSE TDA
    print("PnL MSE TDA")
    losstype = "PnL MSE TDA"
    genPnLMSETDA, discPnLMSETDA, gen_optPnLMSETDA, disc_optPnLMSETDA = TrainLoopMainPnLMSETDAnv(gen, disc, gen_opt, disc_opt,
                                                                                 criterion, alpha, beta, gamma, delta,
                                                                                 lambda_tda, tda_embedding_dim, tda_time_delay,
                                                                                 n_epochs, checkpoint_epoch, train_data,
                                                                                 validation_data, batch_size, hid_d,
                                                                                 hid_g, z_dim, lrd, lrg, h, l, pred,
                                                                                 diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLMSETDA.state_dict()}, f_name + "PnLMSETDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(
        ticker, freq,
        genPnLMSETDA,
        test_data,
        validation_data,
    ...
    plt.figure("Cummulative PnL " + ticker)
    plt.plot(dates_dt[-int(ntest / 2):], np.cumsum(PnL_test[PnL_test_cnt]), label=losstype)
    plt.legend(loc='best')
    PnL_test_cnt += 1
    if (test_data.shape[0] % 2 == 0):
        plt.figure("Intraday cummulative PnL " + ticker)
    ...
    plt.legend(loc='best')
    # endregion: PnL MSE TDA
    # region PnL MSE STD TDA
    print("PnL MSE STD TDA")
    losstype = "PnL MSE STD TDA"
    genPnLMSESTDTDA, discPnLMSESTDTDA, gen_optPnLMSESTDTDA, disc_optPnLMSESTDTDA = TrainLoopMainPnLMSESTDTDAnv(gen, disc, gen_opt,
                                                                                                disc_opt, criterion,
                                                                                                alpha, beta, gamma,
                                                                                                delta, lambda_tda,
                                                                                                tda_embedding_dim,
                                                                                                ...
                                                                                                tanh_coeff, device,
                                                                                                plot)
    torch.save({'g_state_dict': genPnLMSESTDTDA.state_dict()}, f_name + "PnLMSESTDTDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(
        ticker, freq,
        genPnLMSESTDTDA,
        test_data,
    ...
    plt.hist(means_gen, alpha=0.5, bins=100, density=True, stacked=True, label=losstype)
    plt.legend(loc='best')
    # endregion: PnL MSE STD TDA
    # region PnL MSE SR TDA
    print("PnL MSE SR TDA")
    losstype = "PnL MSE SR TDA"
    genPnLMSESRTDA, discPnLMSESRTDA, gen_optPnLMSESRTDA, disc_optPnLMSESRTDA = TrainLoopMainPnLMSESRTDAnv(gen, disc, gen_opt, disc_opt,
                                                                                           criterion, alpha, beta,
                                                                                           gamma, delta, lambda_tda,
                                                                                           tda_embedding_dim,
                                                                                           tda_time_delay,
                                                                                           n_epochs,
                                                                                           checkpoint_epoch, train_data,
                                                                                           validation_data, batch_size,
                                                                                           hid_d, hid_g, z_dim, lrd,
                                                                                           lrg, h, l, pred, diter,
                                                                                           tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLMSESRTDA.state_dict()}, f_name + "PnLMSESRTDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(
        ticker, freq,
        genPnLMSESRTDA,
        test_data,
    ...
    plt.hist(means_gen, alpha=0.5, bins=100, density=True, stacked=True, label=losstype)
    plt.legend(loc='best')
    # endregion PnL MSE SR TDA
    # region PnL SR TDA
    print("PnL SR TDA")
    losstype = "PnL SR TDA"
    genPnLSRTDA, discPnLSRTDA, gen_optPnLSRTDA, disc_optPnLSRTDA= TrainLoopMainPnLSRTDAnv(gen, disc, gen_opt, disc_opt, criterion,
                                                                            alpha, beta, gamma, delta, lambda_tda,
                                                                            tda_embedding_dim, tda_time_delay, n_epochs,
                                                                            checkpoint_epoch, train_data,
                                                                            validation_data, batch_size, hid_d, hid_g,
                                                                            z_dim, lrd, lrg, h, l, pred, diter,
                                                                            tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLSRTDA.state_dict()}, f_name + "PnLSRTDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genPnLSRTDA,
                                                                                                          test_data,
                                                                                                          validation_data,
    ...
    plt.hist(means_gen, alpha=0.5, bins=100, density=True, stacked=True, label=losstype)
    plt.legend(loc='best')
    # endregion PnL SR TDA
    # region PnL STD TDA
    print("PnL STD TDA")
    losstype = "PnL STD TDA"
    genPnLSTDTDA, discPnLSTDTDA, gen_optPnLSTDTDA, disc_optPnLSTDTDA = TrainLoopMainPnLSTDTDAnv(gen, disc, gen_opt, disc_opt,
                                                                                 criterion, alpha, beta, gamma, delta,
                                                                                 lambda_tda, tda_embedding_dim, tda_time_delay,
                                                                                 n_epochs, checkpoint_epoch, train_data,
                                                                                 validation_data, batch_size, hid_d,
                                                                                 hid_g, z_dim, lrd, lrg, h, l, pred,
                                                                                 diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genPnLSTDTDA.state_dict()}, f_name + "PnLSTDTDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(
        ticker, freq,
        genPnLSTDTDA,
        test_data,
    ...
    plt.hist(means_gen, alpha=0.5, bins=100, density=True, stacked=True, label=losstype)
    plt.legend(loc='best')
    # endregion PnL STD TDA
    # region SR TDA
    print("SR TDA")
    losstype = "SR TDA"
    genSRTDA, discSRTDA, gen_optSRTDA, disc_optSRTDA = TrainLoopMainSRTDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta,
                                                             gamma, delta, lambda_tda, tda_embedding_dim, tda_time_delay,
                                                             n_epochs, checkpoint_epoch, train_data,
                                                             validation_data, batch_size, hid_d, hid_g, z_dim, lrd, lrg,
                                                             h, l, pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genSRTDA.state_dict()}, f_name + "SRTDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(
        ticker, freq,
        genSRTDA,
        test_data,
    ...
    print("SR MSE TDA")
    losstype = "SR MSE TDA"
    genSRMSETDA, discSRMSETDA, gen_optSRMSETDA, disc_optSRMSETDA = TrainLoopMainSRMSETDAnv(gen, disc, gen_opt, disc_opt, criterion,
                                                                            alpha, beta, gamma, delta, lambda_tda,
                                                                            tda_embedding_dim, tda_time_delay,
                                                                            n_epochs,
                                                                            checkpoint_epoch, train_data,
                                                                            validation_data, batch_size, hid_d, hid_g,
                                                                            z_dim, lrd, lrg, h, l, pred, diter,
                                                                            tanh_coeff, device, plot)
    torch.save({'g_state_dict': genSRMSETDA.state_dict()}, f_name + "SRMSETDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(
        ticker, freq,
        genSRMSETDA,
        test_data,
    ...
    plt.hist(means_gen, alpha=0.5, bins=100, density=True, stacked=True, label=losstype)
    plt.legend(loc='best')
    # endregion SR MSE TDA
    # region MSE TDA
    print("MSE TDA")
    losstype = "MSE TDA"
    genMSETDA, discMSETDA, gen_optMSETDA, disc_optMSETDA = TrainLoopMainMSETDAnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta,
                                                                  gamma, delta, lambda_tda, tda_embedding_dim, tda_time_delay,
                                                                  n_epochs, checkpoint_epoch, train_data,
                                                                  validation_data, batch_size, hid_d, hid_g, z_dim, lrd,
                                                                  lrg, h, l, pred, diter, tanh_coeff, device, plot)
    torch.save({'g_state_dict': genMSETDA.state_dict()}, f_name + "MSETDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(
        ticker, freq,
        genMSETDA,
        test_data,
    ...
    print("ForGAN TDA")
    losstype = "ForGAN TDA"
    genForGANTDA, discForGANTDA, gen_optForGANTDA, disc_optForGANTDA = TrainLoopForGANTDA(gen, disc, gen_opt, disc_opt,
                                                                                           criterion,
                                                                                           alpha, beta, gamma, delta,
                                                                                           lambda_tda,
                                                                                           tda_embedding_dim,
                                                                                           tda_time_delay, n_epochs,
                                                                                           checkpoint_epoch, train_data,
                                                                                           validation_data, batch_size,
                                                                                           hid_d, hid_g,
                                                                                           z_dim, lrd, lrg, h, l, pred,
                                                                                           diter,
                                                                                           tanh_coeff, device, plot)
    torch.save({'g_state_dict': genForGANTDA.state_dict()}, f_name + "ForGANTDA_generator_checkpoint.pth")
    df_temp, PnL_test[PnL_test_cnt], PnL_even, PnL_odd, means_gen, reals_test, distcheck_test, rl_test = Evaluation2(ticker, freq,
                                                                                                          genForGANTDA,
                                                                                                          test_data,
                                                                                                          validation_data,
    ...
    plt.hist(means_gen, alpha=0.5, bins=100, density=True, stacked=True, label=losstype)
    plt.legend(loc='best')
    # endregion ForGAN TDA

    # region PnL
    print("PnL")
    losstype = "PnL"
    genPnL, discPnL, gen_optPnL, disc_optPnL = TrainLoopMainPnLnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta,
                                                                  gamma, delta, n_epochs, checkpoint_epoch, train_data,
                                                                  validation_data, batch_size, hid_d, hid_g, z_dim, lrd,
                                                                  lrg, h, l, pred, diter, tanh_coeff, device, plot)
    ...
    print("PnL MSE")
    losstype = "PnL MSE"
    genPnLMSE, discPnLMSE, gen_optPnLMSE, disc_optPnLMSE = TrainLoopMainPnLMSEnv(gen, disc, gen_opt, disc_opt,
                                                                                 criterion, alpha, beta, gamma, delta,
                                                                                 n_epochs, checkpoint_epoch, train_data,
                                                                                 validation_data, batch_size, hid_d,
                                                                                 hid_g, z_dim, lrd, lrg, h, l, pred,
    ...
    genPnLMSESTD, discPnLMSESTD, gen_optPnLMSESTD, disc_optPnLMSESTD = TrainLoopMainPnLMSESTDnv(gen, disc, gen_opt,
                                                                                                disc_opt, criterion,
                                                                                                alpha, beta, gamma,
                                                                                                delta, n_epochs,
                                                                                                checkpoint_epoch,
    ...
    plt.hist(means_gen, alpha=0.5, bins=100, density=True, stacked=True, label=losstype)
    plt.legend(loc='best')
    # endregion: PnL MSE STD
    # region PnL MSE SR
    print("PnL MSE SR")
    losstype = "PnL MSE SR"
    genPnLMSESR, discPnLMSESR, gen_optPnLMSESR, disc_optPnLMSESR = TrainLoopMainPnLMSESRnv(gen, disc, gen_opt, disc_opt,
                                                                                           criterion, alpha, beta,
                                                                                           gamma, delta, n_epochs,
                                                                                           checkpoint_epoch, train_data,
                                                                                           validation_data, batch_size,
                                                                                           hid_d, hid_g, z_dim, lrd,
                                                                                           lrg, h, l, pred, diter,
    ...
    plt.hist(means_gen, alpha=0.5, bins=100, density=True, stacked=True, label=losstype)
    plt.legend(loc='best')
    # endregion PnL MSE SR  
    # region PnL SR
    print("PnL SR")
    losstype = "PnL SR"
    genPnLSR, discPnLSR, gen_optPnLSR, disc_optPnLSR = TrainLoopMainPnLSRnv(gen, disc, gen_opt, disc_opt, criterion,
                                                                            alpha, beta, gamma, delta, n_epochs,
                                                                            checkpoint_epoch, train_data,
                                                                            validation_data, batch_size, hid_d, hid_g,
                                                                            z_dim, lrd, lrg, h, l, pred, diter,
                                                                            tanh_coeff, device, plot)
    ...
    print("PnL STD")
    losstype = "PnL STD"
    genPnLSTD, discPnLSTD, gen_optPnLSTD, disc_optPnLSTD = TrainLoopMainPnLSTDnv(gen, disc, gen_opt, disc_opt,
                                                                                 criterion, alpha, beta, gamma, delta,
                                                                                 n_epochs, checkpoint_epoch, train_data,
                                                                                 validation_data, batch_size, hid_d,
                                                                                 hid_g, z_dim, lrd, lrg, h, l, pred,
    ...
    print("SR")
    losstype = "SR"
    genSR, discSR, gen_optSR, disc_optSR = TrainLoopMainSRnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta,
                                                             gamma, delta, n_epochs, checkpoint_epoch, train_data,
                                                             validation_data, batch_size, hid_d, hid_g, z_dim, lrd, lrg,
                                                             h, l, pred, diter, tanh_coeff, device, plot)
    ...
    print("SR MSE")
    losstype = "SR MSE"
    genSRMSE, discSRMSE, gen_optSRMSE, disc_optSRMSE = TrainLoopMainSRMSEnv(gen, disc, gen_opt, disc_opt, criterion,
                                                                            alpha, beta, gamma, delta, n_epochs,
                                                                            checkpoint_epoch, train_data,
                                                                            validation_data, batch_size, hid_d, hid_g,
                                                                            z_dim, lrd, lrg, h, l, pred, diter,
    ...
    print("MSE")
    losstype = "MSE"
    genMSE, discMSE, gen_optMSE, disc_optMSE = TrainLoopMainMSEnv(gen, disc, gen_opt, disc_opt, criterion, alpha, beta,
                                                                  gamma, delta, n_epochs, checkpoint_epoch, train_data,
                                                                  validation_data, batch_size, hid_d, hid_g, z_dim, lrd,
                                                                  lrg, h, l, pred, diter, tanh_coeff, device, plot)
    ...
    print("ForGAN")
    losstype = "ForGAN"
    genfg, discfg, gen_optfg, disc_optfg = TrainLoopForGAN(gen, disc, gen_opt, disc_opt, criterion, alpha, beta, gamma,
                                                           delta, n_epochs, checkpoint_epoch, train_data,
                                                           validation_data, batch_size, hid_d, hid_g, z_dim, lrd, lrg,
                                                           h, l, pred, diter, tanh_coeff, device, plot)
